...
/**
 * Processes matched users with atomic Redis operations using Lua scripts
 * Optimized for memory efficiency and batch processing
 */
...
const processMatchBatchWithLua = async (matches: Match[]): Promise<void> => {
  ...
  for (const match of matches) {
    const user1Key = MatchmakingQueueService.composeKey({
      socketId: match.user1.socketId,
      userId: match.user1.userId,
    });
    const user2Key = MatchmakingQueueService.composeKey({
